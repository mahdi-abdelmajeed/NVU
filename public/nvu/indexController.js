//progress controller
app.controller('progress',function($scope,$rootScope, $interval) {
...
app.controller("tools", function() {
  ...
  this.cscrolling = function() {
    $("#scrolling").toggleClass("scrolling");
  }
});


app.controller('myCtrl', function($scope, $mdTheming) {
  if(Cookie.getCookie('dynamicTheme'))
    $scope.dynamicTheme = Cookie.getCookie('dynamicTheme');
  else
    $scope.dynamicTheme = "default";
  $mdTheming.setBrowserColor({
    theme: $scope.dynamicTheme, // Default is 'default'
...
app.controller('RightCtrl', function ($scope, $mdSidenav, $log) {
...
app.controller('veiwController', function($scope, $interval) {
  var contant = angular.element(document.querySelector('#veiw')).bind('scroll', function(){

  $scope.top= function() {
    $interval(function() {
      contant[0].scrollTop -= 100;
    }, 1, contant[0].scrollTop/100);
  }
